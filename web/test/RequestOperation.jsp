...
<jsp:useBean id="ODocUploadPDF" class="weaver.odoc.workflow.request.ODocUploadPDF" scope="page" />
...
String rand = "";
String hadUploadPDF = fu.getParameter("hadUploadPDF");
...
int docid = Util.getIntValue(fu.getParameter("docid"),-1);
...
if((requestid>0&&(!IsCanSubmit&&!coadCanSubmit)) || RequestManager.checkNodeOperatorComment(requestid,user.getUID(),nodeid)){
    ...
    if(method.indexOf("crenew_") > -1){
		...
		docid = Util.getIntValue(fu.getParameter("docValue"));
...
if(!"save".equals(src)||isremark!=1){
	...
	if("n".equals(eh_setoperator)){		//异常处理窗口未选择操作者，以提交失败处理
		...
		if(!flowstatus && RequestManager.isNeedChooseOperator()){		//流转异常处理，找不到操作者且后台设置了提交至指定人员
			out.println("<script>");
			out.println(prefix+"wfforward('/workflow/request/ViewRequest.jsp?f_weaver_belongto_userid="+userid+"&f_weaver_belongto_usertype="+f_weaver_belongto_usertype+"&requestid="+requestid+"&ehnextnodeid="+RequestManager.getNextNodeid()+"&needChooseOperator=y');");
			out.println("</script>");
			return;
		}
	}
...
//if(flowstatus){
 String fromofs = Util.null2String((String)session.getAttribute("fromofs_"+requestid+""));
  String ofsurl = Util.null2String((String)session.getAttribute("ofsurl_"+requestid+""));
   /*if("1".equals(fromofs)&&!ofsurl.equals("")){
	   String url = Util.null2String(RecordSet.getPropValue("requestsenddata","url"))+"/workflow/request/ViewRequestOS.jsp?reflush=1&timestamp="+new Date().getTime();
	   RecordSet.writeLog("异构系统回调：URl="+url);
	   response.sendRedirect(url);
	   out.print("<script>"+prefix+"wfforward('"+ofsurl+"');</script>");
	   return ;
   }else{
	   RecordSet.writeLog("异构系统回调：fromofs="+fromofs+";ofsurl:"+ofsurl);
   }*/
				String url = Util.null2String(RecordSet.getPropValue("requestsenddata","url"))+"/workflow/request/ViewRequestOS.jsp?reflush=1&timestamp="+new Date().getTime();
				RecordSet.writeLog("异构系统回调11：flowstatus="+flowstatus);
				RecordSet.writeLog("异构系统回调11：fromofs="+fromofs+";ofsurl:"+ofsurl+";url:"+url);
				RecordSet.writeLog("异构系统回调11：fromofs="+ session.getAttributeNames().toString());
				url = "http://60.10.65.80:12982/workflow/request/ViewRequestOS.jsp?reflush=1&timestamp="+new Date().getTime();
				response.sendRedirect(url);
				out.print("<script>"+prefix+"wfforward('"+ofsurl+"');</script>");
				return ;
//}
...
//add by fanggsh 2007-03-23  for TD6247  调整为：在流程中以TAB页形式打开相关文档时才保存流程与模板对应数据  结束
		...
		if (flowDocs!=null&&flowDocs.size()>0){
           ...
           if (TempAyyay!=null&&TempAyyay.size()>0)
              {
                 secid=""+TempAyyay.get(TempAyyay.size() - 1);
              }
      }
      if(flowViewDocs!=null&&flowViewDocs.size()>0){
        secid=""+flowViewDocs.get(flowViewDocs.size() - 1);
      }
