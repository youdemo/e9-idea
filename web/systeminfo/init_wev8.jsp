...
<head>
    ...
    <%
        ...
        if (licenseCheckLogin != null && user != null) {
...
//问题1
    ...
    */
    ...
    <%
        ...
        if(useragent.contains("Firefox")||useragent.contains(" Chrome")||useragent.contains("Safari") || useragent.contains("Edge")){
            isIE = "false";
        }
        else{
            isIE = "true";
...
//TD2125 by mackjoe 解决数据中心登陆不了问题
        ...
        if (false && "1".equals(user.getLogintype())
                && !relogin0.equals("1")
                && !fromlogin.equals("yes")
                && !frommain.equals("yes")
                && !s_logmessage.equals(a_logmessage)
                && !"true".equals(rtxFromLogintmp)
                && !"1".equals(user.getAccount_type())) {
    ...
    <%
	//表单设计页面需要使用特殊版本的jQuery
	if(request.getRequestURI().indexOf("/workflow/exceldesign/") != -1 
	|| request.getRequestURI().indexOf("/fna/exceldesign/") != -1 ){
	%>
		<script type="text/javascript" src="/wui/common/jquery/jquery.min_other.js"></script>
	<%
	//流程的三个页面需要使用高版本的jQuery
	}else if (request.getRequestURI().indexOf("/workflow/request/") != -1 &&
