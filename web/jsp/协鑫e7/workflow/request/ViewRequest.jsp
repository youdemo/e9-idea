...
if(ismonitor==1){
	...
	String cansjck = "";
	String isfromsjjk=Util.null2String(request.getParameter("isfromsjjk"));
	if("1".equals(isfromsjjk)) {
		...
		sql = "select max(id) as id from " + tablename_sz + " where ','||to_char(sjry)||',' like '%," + userid + ",%'";
		String pzids = "";
		rs.executeSql(sql);
		if (rs.next()) {
		...
		if (!"".equals(pzids)) {
			...
			sql = "select requestid from workflow_requestbase where workflowid in(select workflowid from " + tablename_jl + " where sjszid=" + pzids + ") and createdate>='" + startdate + "' and createdate<='" + enddate + "' and creater in(select resourceid from hrmrolemembers where roleid in(" + sjdx + ")) and requestid=" + requestid;
			rs.executeSql(sql);
			if (rs.next()) {
				cansjck = "1";
			}
