create or replace function f_fna_tl_getHotelCost(i_begindate in varchar2,i_enddate in varchar2,i_travelgroup  in varchar2,
...
begin
  ...
  if i_begindate = '' or i_enddate = '' or i_travelgroup= '' or i_gncldj = '' or i_gwcldj = '' or i_mdd='' or i_rswf ='' or i_clfa = '' or i_clfa != '10' then
    ...
    if i_type = '0' then
        select round(v_betrg*v_xs,2) into v_result from dual;
    else
            select round(v_betrg*v_xs*(TO_DATE(i_enddate, 'YYYY-MM-DD') - TO_DATE(i_begindate, 'YYYY-MM-DD')),2) into v_result from dual;
    end if;
  end if;
  return v_result;
end;
