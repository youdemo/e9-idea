...
i_zc in number,i_wuc in number,i_wac in number,i_zf in number,i_cfzk in number,i_begintime in varchar2,i_endtime in varchar2,i_jtcheck in varchar2,i_jt in varchar2)
    ...
    if i_zfcheck = '1' or i_jtcheck = '1'  then
        if i_begintime > '12:00' or i_endtime<='12:00' then
            v_zfje := nvl(v_zfje,0.0)/2*nvl(i_cfzk,1);
             ...
             v_zfje := nvl(v_zfje,0.0)*nvl(i_cfzk,1);
        end if;
    end if;
