create or replace function f_fna_tl_getHotelCostHL(i_begindate in varchar2,i_enddate in varchar2,i_travelgroup  in varchar2,
i_gncldj in varchar2,i_gwcldj in varchar2,i_mdd in varchar2,i_rswf in varchar2,i_clfa in varchar2,i_type in varchar2,i_sfmyzf  in varchar2)
--i_begindate 开始日期 i_enddate 结束日期 i_travelgroup 差旅组 i_gnw 国内外 i_gncldj国内差旅等级
--i_gwcldj 国外差旅等级 i_mdd 城市别 i_rswf 人事范围 i_clfa 差旅方案 i_type类型 0 单日 1 汇总 i_sfmyzf 是否美元支付
return  number is
...
begin
  ...
  if i_begindate = '' or i_enddate = '' or i_travelgroup= '' or i_gncldj = '' or i_gwcldj = '' or i_mdd='' or i_rswf ='' or i_clfa = '' or i_clfa != '10' or i_sfmyzf = '' then
    v_result :=0.00;
  else
    ...
    elsif i_sfmyzf ='1' then
      select rate into v_exchange_rate from uf_fna_Curr where curr = v_curr and tcurr='USD';
    end if;
    if i_type = '0' then
        select round(v_betrg*v_xs*v_exchange_rate,2) into v_result from dual;
    else
            select round(v_betrg*v_xs*v_exchange_rate*(TO_DATE(i_enddate, 'YYYY-MM-DD') - TO_DATE(i_begindate, 'YYYY-MM-DD')),2) into v_result from dual;
    end if;
  end if;
  return v_result;
end;
