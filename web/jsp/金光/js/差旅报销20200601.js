...
jQuery(document).ready(function(){
    ...
    //住宿费明细超标准校验
    var changedt4field = HOCOSTRMB_dt5+","+TotStaAmo_dt5;
    WfForm.bindDetailFieldChangeEvent(changedt4field,function(id,rowIndex,value){
        // var HOCOST_dt5_val = WfForm.getFieldValue(HOCOSTRMB_dt5+"_"+rowIndex);
            ...
            if (arr_dt2.length > 0) {
                for (var i = 0; i < arr_dt2.length; i++) {
                    ...
                    if(fpsfcf_dt2_val !="" && Number(fpsfcf_dt2_val)>Number("0")){
                        alert("发票号码("+TraINVNUM_dt2_val+") 已被其他流程使用，请检查");
                        return;
                    }
                    if(TraINVNUM_dt2_val != ""){
                        ...
                        if(result == "1"){
                            alert("发票号码("+TraINVNUM_dt2_val+") 在当前单据存在重复，请检查");
                            return;
                        }
                    }
                }
            ...
            if (arr_dt3.length > 0) {
                for (var i = 0; i < arr_dt3.length; i++) {
                    ...
                    if(fpsfcf_dt3_val !="" && Number(fpsfcf_dt3_val)>Number("0")){
                        alert("发票号码("+OTHINVNUM_dt3_val+") 已被其他流程使用，请检查");
                        return;
                    }
                    if(OTHINVNUM_dt3_val != ""){
                        ...
                        if(result == "1"){
                            alert("发票号码("+OTHINVNUM_dt3_val+") 在当前单据存在重复，请检查");
                            return;
                        }
                    }
                }
            ...
            if (arr_dt5.length > 0) {
                for (var i = 0; i < arr_dt5.length; i++) {
                    ...
                    if(fpsfcf_dt5_val !="" && Number(fpsfcf_dt5_val)>Number("0")){
                        alert("发票号码("+HOINVNUM_dt5_val+") 已被其他流程使用，请检查");
                        return;
                    }
                    if(HOINVNUM_dt5_val != ""){
                        ...
                        if(result == "1"){
                            alert("发票号码("+HOINVNUM_dt5_val+") 在当前单据存在重复，请检查");
                            return;
                        }
                    }
