            ...
            ,function(){
              if(options.showLine){
                if(hasChild || item.lazy){ //lazy 是否开启懒加载，需要配合spread事件使用
                  ...
                  return '<span class="layui-tree-iconClick"><i class="layui-icon layui-icon-home"></i></span>';
                };
              }else{
      ...
      if(type == 'add'){
        ...
        if(!packCont[0]){
          ...
          if(options.showLine){
			...
			elemMain.find('.'+ICON_CLICK).children('.layui-icon').addClass(ICON_ADD).removeClass('layui-icon-home');
        ...
        layer.confirm('确认删除该节点 "<span style="color: #999;">'+ (item.title || '') +'</span>" 吗？', function(index){
          ...
          if(elem.siblings('.'+ELEM_SET).children('.'+ELEM_ENTRY)[0]){
            ...
            if(options.showLine){
              ...
              prevDiv.find('.'+ICON_CLICK).children('.layui-icon').removeClass(ICON_SUB).addClass('layui-icon-home');
              //父节点所在层添加延伸线
