...
public class UpdatePickingApproverAction extends BaseBean implements Action {
    @Override
    public String execute(RequestInfo info) {
        XuzhouWorkflowUtil workflowUtil = new XuzhouWorkflowUtil();
        RecordSet rs = new RecordSet();
        String requestid = Util.null2String(info.getRequestid());
        this.writeLog("获取领料矩阵审批人 Action开始：" + requestid);
        ...
        if (!"".equals(tableName)) {
            sql = " select BMFZR,FGLD from " + getMatrixName("半导体部门矩阵") + " where XJBMWD in (select DEPARTMENTID from HRMRESOURCE " +
                    " where id in (select CREATER from WORKFLOW_REQUESTBASE where requestid=" + requestid + " )) ";
        ...
        this.writeLog("获取领料矩阵审批人 Action完成：" + requestid);
