 ...
 * desc 加班情况报表数据
 */
public class GetJbqkdataService20200521 {
    @POST
    @Path("/getYdljjbqkmx")
    ...
    public String getData(@Context HttpServletRequest request, @Context HttpServletResponse response) {
        ...
        String sql = "select t.*,t.PS_HOURS + t.ZM_HOURS + t.FD_HOURS as total_hours," +
                "       round((t.PS_HOURS + t.ZM_HOURS + t.FD_HOURS) / t.st_tops, 4) * 100 || '%' as rate, " +
                "       round((t.PS_HOURS + t.ZM_HOURS + t.FD_HOURS) / t.up_tops, 4) * 100 || '%' as rate1  " +
                "from ( " +
                "select b.workcode," +
                "               b.name," +
                "               b.companyname," +
                ...
                "                   and overtime_type like '%平时%') AS PS_HOURS, " +
                "               (SELECT nvl(SUM(nvl(a_hours, 0)), 0) " +
                "                  from formtable_main_744_dt1 aa " +
                "                 where cday >= '"+begindate+"' " +
                "                   and cday <= '"+enddate+"' " +
                "                   and aa.work_code = b.WORKCODE " +
                "                   and overtime_type like '%周末%') AS ZM_HOURS, " +
                "               (SELECT nvl(SUM(nvl(a_hours, 0)), 0) " +
                "                  from formtable_main_744_dt1 aa " +
                "                 where cday >= '"+begindate+"' " +
                "                   and cday <= '"+enddate+"' " +
                "                   and aa.work_code = b.WORKCODE " +
                "                   and overtime_type like '%法定%') AS FD_HOURS, " +
                ...
                "                 F_getovertime('"+enddate+"', a.workcode, 'G') AS up_tops " +
         ...
         if(!"".equals(orgcode)){
            sql=sql+" and currdeptcode in("+orgcode+")";
         }
        ...
        if(!"".equals(jbsdqsstart)){
            sql=sql+" and (SELECT nvl(SUM(nvl(a_hours, 0)), 0)" +
                    "                        from formtable_main_744_dt1 aa" +
                    "                       where cday >= '"+begindate+"'" +
                    "                         and cday <= '"+enddate+"'" +
                    "                         and aa.work_code = a.WORKCODE" +
                    "                         and (overtime_type like '%法定%' or overtime_type like '%平时%' or overtime_type like '%周末%'))>="+jbsdqsstart;
        }
        ...
        if(!"".equals(jbsdqsend)){
            sql=sql+" and (SELECT nvl(SUM(nvl(a_hours, 0)), 0)" +
                    ...
                    "                         and cday <= '"+enddate+"'" +
                    "                         and aa.work_code = a.WORKCODE" +
                    "                         and (overtime_type like '%法定%' or overtime_type like '%平时%' or overtime_type like '%周末%'))<="+jbsdqsend;
        }

         sql += " order by id asc) t ) b, " +
                "       hrmresource a " +
    ...
    public String getNdljData(@Context HttpServletRequest request, @Context HttpServletResponse response) {
        ...
        String sql = "select t.*,(yue1+yue2+yue3+yue4+yue5+yue6+yue7+yue8+yue9+yue10+yue11+yue12)/(trunc(MONTHS_BETWEEN(to_date('"+nowdate+"','yyyy-mm-dd'),to_date('"+month1+"','yyyy-mm-dd')))+1) as yjjbss," +
    ...
    public InputStream ndOutExcel(HttpServletRequest request, HttpServletResponse response) {
        ...
        try{
            ...
            CellRangeAddress cra = new CellRangeAddress(0,0,10,21);
            sheet.addMergedRegion(cra);
            ...
            String month11 = year+"-11-01";
            String month12 = year+"-12-01";
            sql = "select t.*,(yue1+yue2+yue3+yue4+yue5+yue6+yue7+yue8+yue9+yue10+yue11+yue12)/(trunc(MONTHS_BETWEEN(to_date('"+nowdate+"','yyyy-mm-dd'),to_date('"+month1+"','yyyy-mm-dd')))+1) as yjjbss," +
                    "(yue1+yue2+yue3+yue4+yue5+yue6+yue7+yue8+yue9+yue10+yue11+yue12) as yljjbss" +
                    " from (" +
                    "select b.workcode," +
                    "               b.name," +
