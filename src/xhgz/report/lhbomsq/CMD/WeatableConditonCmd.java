...
//        }
        ...
        SearchConditionItem requestid = conditionFactory.createCondition(ConditionType.INPUT,502327, "kzlh");
        ...
        requestid.setLabel("料号"); //设置文本值 这个将覆盖多语言标签的值
        //workcode.setRules("required");
        // workcode.setRules("required");
        // workcode.setIsQuickSearch(true);
        conditionItems.add(requestid);

        SearchConditionItem requestid1 = conditionFactory.createCondition(ConditionType.INPUT,502327, "lh");
        requestid1.setColSpan(2);//定义一行显示条件数，默认值为2,当值为1时标识该条件单独占一行
        requestid1.setFieldcol(16); //条件输入框所占宽度，默认值18
        requestid1.setLabelcol(8);
        requestid1.setViewAttr(2); // 编辑权限 1：只读，2：可编辑， 3：必填 默认2
        requestid1.setLabel("列号"); //设置文本值 这个将覆盖多语言标签的值
        //workcode.setRules("required");
        // workcode.setRules("required");
        // workcode.setIsQuickSearch(true);
        conditionItems.add(requestid1);
