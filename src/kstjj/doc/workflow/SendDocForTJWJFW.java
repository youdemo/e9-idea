 ...
 * 统计文件发文流程 推送公文到政府oa
 */
public class SendDocForTJWJFW implements Action {
    @Override
    public String execute(RequestInfo info) {
        ...
        log.writeLog("SendDocForTJWJFW","开始推送公文 requestid："+requestid);
        String sql = "select * from "+tableName+" where requestid="+requestid;
        rs.execute(sql);
        if(rs.next()){
            title = Util.null2String(rs.getString("bt"));
            code = Util.null2String(rs.getString("fwh"));
        ...
        String flag = "";
        if(!"".equals(zsdw)){
            sql = "select guid,bmmc from uf_inforesrdepet where bmmc in(select dwmc from uf_oabill where id in("+getFwdw(zsdw)+"))";
            rs.execute(sql);
            while(rs.next()){
...
//        if(!"".equals(csdw)){
//            sql = "select guid,bmmc from uf_inforesrdepet where bmmc in(select dwmc from uf_oabill where id in("+getFwdw(csdw)+"))";
//            rs.execute(sql);
//            while(rs.next()){
//                swbm = swbm + flag + Util.null2String(rs.getString("guid"));
//                swbmmc = swbmmc + flag + Util.null2String(rs.getString("bmmc"));
...
//        }
        if("1".equals(wsfboa)){
            ...
            try {
                ...
                log.writeLog("SendDocForTJWJFW",e);
                info.getRequestManager().setMessagecontent("推送公文失败，请联系系统管理员");
                info.getRequestManager().setMessageid("ErrorMsg:");
                return FAILURE_AND_CONTINUE;
