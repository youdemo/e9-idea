	...
	public void doPlanData(String billid,int count,String planid) throws Exception {
		...
		if("2".equals(type)) {
			...
			if(rs.next()) {
				...
				log.writeLog(this.getClass().getName(), "testex description:"+description);
				description = getZw(description);
				log.writeLog(this.getClass().getName(), "testex trans:"+description);
				oaam.setBody(description);
