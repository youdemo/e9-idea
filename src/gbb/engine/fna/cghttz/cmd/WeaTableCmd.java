    ...
    public Map<String, Object> execute(CommandContext commandContext) {
       ...
       // }
        ...
        try {
            ...
            String fileds = " id,htmc,htbh,cgsqname,cgsq,(select gysmc from uf_gyslb where id=a.htxdf)htxdf,htje,spzje,yfkje,wfkje,(select t2.selectname from workflow_billfield t, workflow_bill t1,workflow_selectitem t2 where t.billid=t1.id and t2.fieldid=t.id  and t1.tablename='uf_cghttz' and t.fieldname='cgjelx' and t2.selectvalue=a.cgjelx) as cgjelx  ";
            ...
            table.setSqlform(" (select id,htmc,htbh,cgjelx,(select requestname from workflow_requestbase where requestid=a.cgsq ) as cgsqname,cgsq,htxdf,cast(htje as numeric(10,2)) as htje,cast((select isnull(sum(isnull(t.bcfkje,0)),0) from formtable_main_52 t,workflow_requestbase t1 where t.requestid=t1.requestid and t1.currentnodetype in(1,2)and t.cghtmc=a.id ) as numeric(10,2)) as spzje,cast((select isnull(sum(isnull(t.bcfkje,0)),0) from formtable_main_52 t,workflow_requestbase t1 where t.requestid=t1.requestid and t1.currentnodetype in(3)and t.cghtmc=a.id ) as numeric(10,2)) as yfkje,cast((htje-(select isnull(sum(isnull(t.bcfkje,0)),0) from formtable_main_52 t,workflow_requestbase t1 where t.requestid=t1.requestid and t1.currentnodetype in(1,2,3)and t.cghtmc=a.id )) as numeric(10,2)) as wfkje from uf_cghttz a) a ");
            ...
            table.getColumns().add(new WeaTableColumn("10%", "合同相对方", "htxdf","htxdf"));
            table.getColumns().add(new WeaTableColumn("10%", "采购金额类型", "cgjelx","cgjelx"));
