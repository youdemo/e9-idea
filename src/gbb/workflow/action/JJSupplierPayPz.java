 ...
 * 间接供应商付款 凭证冲销
 */
public class JJSupplierPayPz implements Action {
    @Override
    public String execute(RequestInfo info) {
        new BaseBean().writeLog("TravelReimPZ start");
        ...
        if(rs.next()){
            ...
            djbh = Util.null2String(rs.getString("bdbh"));
            fkgsbm = Util.null2String(rs.getString("fkgsbm"));
            cbzxbm = Util.null2String(rs.getString("cbzxbm"));
            sfdk = Util.null2String(rs.getString("sfdk"));
            wbsxm = Util.null2String(rs.getString("wbsxm"));
        }
        if(djbh.length()>13){
        ...
        int num = 1;
        sql = "select * from "+tableName+"_dt1 where mainid="+mainid+" order by id asc";
        rs.execute(sql);
        while(rs.next()){
            ...
            map.put("LIFNR",gysbh);//供应商编码
            map.put("RACCT",Util.null2String(rs.getString("kmbm")));//费用科目
            map.put("KOSTL", Util.null2String(rs.getString("cbzxbm"))+fkgsbm);//成本中心
            map.put("PS_POSID",wbsxm);//WBS编码
            map.put("KUNNR","");//中心号
            map.put("BANFN","");//采购申请号
            map.put("WAERS","CNY");//币种
            map.put("TSL",Util.null2String(rs.getString("bhsje")));//不含税金额
            map.put("TSL2",Util.null2String(rs.getString("se")));//税额
            String tax = "N";
            if("0".equals(sfdk)){
                tax="Y";
            }
            map.put("TAX",tax);//是否抵扣
    ...
    public  Map<String,String> doService(List<Map<String,String>> list)  {
       ...
       log.writeLog("TravelReimPZ start");
       ...
       try {
           ...
           String sysidmt = Util.null2o(weaver.file.Prop.getPropValue("sapmt", "sysid"));
            ...
            for(Map<String,String> map :list) {
                log.writeLog("TravelReimPZ map:"+ JSONObject.toJSONString(map).toString());
           ...
           log.writeLog("TravelReimPZ result:"+ JSONObject.toJSONString(resultMap).toString());

       }catch(Exception e){
            new BaseBean().writeLog("TravelReimPZ",e);
        }
        return resultMap;
