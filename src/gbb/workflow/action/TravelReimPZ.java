 ...
 * 差旅费用报销流程 凭证冲销
 */
public class TravelReimPZ implements Action {
    @Override
    public String execute(RequestInfo info) {
        ...
        rs.execute(sql);
        if(rs.next()){
            ccrgh = tu.getWorkcode(Util.null2String(rs.getString("ccr")));
        ...
        int num = 1;
        sql = "select * from "+tableName+"_dt4 where mainid="+mainid+" order by id asc";
        rs.execute(sql);
        while(rs.next()){
