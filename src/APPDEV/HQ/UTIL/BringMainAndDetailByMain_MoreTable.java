 ...
 *
 */
public class BringMainAndDetailByMain_MoreTable {
    public static BaseBean basebean = new BaseBean();
    private String dateid;


    public BringMainAndDetailByMain_MoreTable() {
        this.dateid = "1";
    }

    public BringMainAndDetailByMain_MoreTable(String dateid) {
        this.dateid = dateid;
    }
     ...
     * 传入参数，并获取结果集
     *
     * @param oaDatas    Map<OA字段,页面上对应的值>
     * @param workflowID
     * @return
     */
    public String getReturn(Map<String, String> oaDatas, String workflowID, List<String> inTableName, Map<String,List<Map<String, String>>> datamaps) {
        String formid = "-1";
        ...
        try {
            ...
            // add by shaw 2018-12-07 14:37:33
            // String inputtype = getInputType(workflowID);
            List<String> stringList = getInputType(workflowID);
            for (String inputtype : stringList) {
                // basebean.writeLog("inputtype:" + inputtype);
                if ("inTab".equals(inputtype)) {
                    for(String tablename:inTableName){
                        JCO.Table jt = bapi.getTableParameterList().getTable(tablename);
                        List<Map<String, String>> list = datamaps.get(tablename);
                    ...
                    for(String tablename:inTableName){
                        JCO.Structure strlist = paraList.getStructure(tablename);
                    ...
                    while (it.hasNext()) {
                        ...
                        try {
                            ...
                            // basebean.writeLog("type:" + bean.getSAPParameterType());
                    ...
                    // basebean.writeLog("paraList：" + paraList.toString());
                }
            }
     ...
     * @return
     */
    public String getReturnNew(Map<String, String> oaDatas, String workflowID, List<Map<String, Map<String, String>>> structList, List<Map<String, List<Map<String, String>>>> tableList) {
        String formid = "-1";
        basebean.writeLog("---------------------start SAP OA MAPPING ----------------------------");
        JSONObject jsonObj = new JSONObject();
        JSONObject jsonTable = new JSONObject();
        ...
        try {
            ...
            for (String inputtype : stringList) {
                // basebean.writeLog("inputtype:" + inputtype);
                if ("inTab".equals(inputtype)) {
                    for (Map<String, List<Map<String, String>>> list : tableList) {
                        Set<Map.Entry<String, List<Map<String, String>>>> mainKeySet = list.entrySet();
                        Iterator<Map.Entry<String, List<Map<String, String>>>> iterator = mainKeySet.iterator();
                        if (iterator.hasNext()) {
                            Map.Entry<String, List<Map<String, String>>> entry = iterator.next();
                            String inTableName = entry.getKey();
                            JCO.Table jt = bapi.getTableParameterList().getTable(inTableName);
                            List<Map<String, String>> lists = list.get(inTableName);
                            int length = lists.size();
                            // basebean.writeLog("length" + length);
                            for (int i = 0; i < length; i++) {
                                Map<String, String> oadata = lists.get(i);
                                jt.appendRow();
                                jt.setRow(i);
                                Set keys1 = oadata.keySet();
                                Iterator it1 = keys1.iterator();
                    ...
                    for (Map<String, Map<String, String>> mapMap : structList) {
                        Set<Map.Entry<String, Map<String, String>>> mainKeySet = mapMap.entrySet();
                        Iterator<Map.Entry<String, Map<String, String>>> iterator = mainKeySet.iterator();
                        if (iterator.hasNext()) {
                            Map.Entry<String, Map<String, String>> entry = iterator.next();
                            String inStrName = entry.getKey();
                            JCO.Structure strlist = paraList.getStructure(inStrName);
                            Map<String, String> structMap = mapMap.get(inStrName);

                            Set<String> keys = structMap.keySet();
                            Iterator<String> it = keys.iterator();
                            while (it.hasNext()) {
                                String oaFieldName = it.next();
                                String value = structMap.get(oaFieldName);
                                // basebean.writeLog("struct value" + value + " |name " + oaFieldName);
                                strlist.setValue(value, oaFieldName);
                            }
                        }
                    }
     ...
     * @return
     */
    public String getReturnObj(Map<String, Object> oaDatas, String workflowID, String inTableName, List<Map<String, String>> list) {
        String formid = "-1";
        basebean.writeLog("---------------------start SAP OA MAPPING ----------------------------");
        JSONObject jsonObj = new JSONObject();
        JSONObject jsonTable = new JSONObject();
        ...
        try {
            ...
            for (String inputtype : stringList) {
                ...
                if ("inTab".equals(inputtype)) {
                    ...
                    while (it.hasNext()) {
                        String oaFieldName = it.next();
                        String value = oaDatas.get(oaFieldName).toString();
                        strlist.setValue(value, oaFieldName);
                    }
                } else {
                    Set<String> keys = oaDatas.keySet();
    ...
    private static List<String> getInputType(String workflowid) {
        RecordSet rs = new RecordSet();
        ...
        while (rs.next()) {
            SAPParameterType = Util.null2String(rs.getString("SAPParameterType"));
            if (!list.toString().contains(SAPParameterType)) {
                list.add(SAPParameterType);
            }
        }
        return list;
    }
     ...
     */
    private static JSONArray getReturnValues2(JCO.Function bapi,
                                              List<SapOaMappingBean> list, String formid, String tabName) throws JSONException {
        JSONArray arr = new JSONArray();
        // 获取返回的结果
        JCO.ParameterList outTable = bapi.getTableParameterList();
        for (SapOaMappingBean outBean : list) {
            ...
            if ("outPara".equals(outBean.getSAPParameterType())) {
                ...
                // 处理指定输出表
                // JCO.Table jt = outTable.getTable(outBean.getSAPParameterName());
                JCO.Table jt = outTable.getTable(tabName);
                basebean.writeLog("SAP输出表的名称：" + outBean.getSAPParameterName());
                basebean.writeLog("获取返回的行数：" + jt.getNumRows());
                if (jt.getNumRows() > 0) {
                    for (int i = 0; i < jt.getNumRows(); i++) {
