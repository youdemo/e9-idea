...
public class AutoRequestService extends BaseBean {
     ...
     * @param strJson
     *            格式说明 ：
     *            例子：{"DETAILS":{"DT1":[{"b1":"2","a1":"1"},{"b1":"22","a1":"12"
     *            }],"DT2":[{"b2":"22","a2":"11"}]},"HEADER":{"aa":"aa","bb":
     *            "bb"}} HEADER：存放主表信息 （"字段名":"值"） DETAILS存放明细表信息 明细1
     *            以DT1标识的array 明细2以DT2标识 的array以此类推
     * @param createrid
     *            流程创建人id
     * @param isNext
     *            是否流转到下一节点 1 是 0 不是
     * @return
     */
    public String createRequest(String workflowCode, String strJson, String createrid, String isNext) {
        ...
        String sql = "select count(1) as count from workflow_base where id=" + workflowID;
        rs.execute(sql);
        int count = 0;
        if (rs.next()) {
        ...
        if (creater.length() > 0 && !"1".equals(creater)) {
            sql = "select count(1) as count_cc from hrmresource " + "where id='" + creater + "' and status in(0,1,2,3)";
            rs.execute(sql);
            if (rs.next()) {
    ...
    private DetailTableInfo getDetailTableInfo(String jsonStr) throws Exception {
        ...
        for (int j = 1; j <= length; j++) {
            ...
            for (int i = 0; i < arr.length(); i++) {
                ...
                for (int index = 0; index < list_cell.size(); index++) {
                    cells[index] = (Cell) list_cell.get(index);
                }
                row.setCell(cells);
                row.setId("" + i);
                list_row.add(row);
            ...
            for (int index = 0; index < list_row.size(); index++) {
                ...
                rows[index] = (Row) list_row.get(index);
            }
            dt.setRow(rows);
            dt.setId("" + j);
            list_detail.add(dt);
        ...
        for (int index = 0; index < list_detail.size(); index++) {
            detailtables[index] = (DetailTable) list_detail.get(index);
        }
    ...
    private MainTableInfo getMainTableInfo(String jsonStr) throws Exception {
        ...
        for (int index = 0; index < list.size(); index++) {
            properties[index] = (Property) list.get(index);
        }
    ...
    private void insertnownode(String requestid) {
        ...
        if (count == 0) {
            sql = "insert into workflow_nownode(requestid,nownodeid,nownodetype,nownodeattribute) values('" + requestid
                    + "','219',0,0)";
