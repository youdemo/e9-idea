...
public class FlightOrderDataServcieImpl extends Service implements FlightOrderDataServcie {
...
//                cell.setCellStyle(sheetStyle2);
    ...
    public String createRequest(String clsqid,String lastmonth) throws JSONException {
        ...
        sql = "select nvl(sum(nvl(amount,0)),0) as refund from ctrip_SettlemenFlightOrdert where orderid in (select orderid from ctrip_SettlemenFlightOrdert where orderdetailtype='退票' and DefineFlag="+clsqid+" and FlightClass='N' and OrderType='1' and substr(AccCheckBatchNo,instr(AccCheckBatchNo,'_',-1)+1,6) ='"+lastmonth+"')";
        rs.execute(sql);
        if(rs.next()){
...
//        }
        ...
        sql = "select id,DefineFlag,OrderID,PassengerName,TakeOffTime||' '||TakeOffTime2 as TakeOffTime,case when FlightClass='I' then '国际' when FlightClass='N' then '国内' else '' end as  FlightClass," +
                ...
                "RefundServiceFee,Refund,Amount as RealAmount,ItineraryFee from ctrip_SettlemenFlightOrdert where  DefineFlag='"+clsqid+"' and FlightClass='N' and OrderType='1' and substr(AccCheckBatchNo,instr(AccCheckBatchNo,'_',-1)+1,6) ='"+lastmonth+"'";
        rs.execute(sql);
        while(rs.next()){
            JSONObject jo = new JSONObject();
