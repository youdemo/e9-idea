package APPDEV.HQ.ENGINE.FNA.TL.FILGHTGW.WEB;

import APPDEV.HQ.ENGINE.FNA.TL.FILGHTGW.SERVICE.FlightOrderDataGWServcie;
import APPDEV.HQ.ENGINE.FNA.TL.FILGHTGW.SERVICE.IMPL.FlightOrderDataServcieGWImpl;
 ...
 * 机票对账列表
 **/
public class FlightOrderDataGWAction {

    private FlightOrderDataGWServcie getService(User user) {
        return (FlightOrderDataServcieGWImpl) ServiceUtil.getService(FlightOrderDataServcieGWImpl.class, user);
    }
    ...
    @Path("/createtplc")
    @Produces({MediaType.TEXT_PLAIN})
    public String createTpLc(@Context HttpServletRequest request, @Context HttpServletResponse response){
        Map<String, Object> apidatas = new HashMap<String, Object>();
        String result = "";
        //获取当前用户
        User user = HrmUserVarify.getUser(request, response);
        result = getService(user).createTpLc(ParamUtil.request2Map(request));
