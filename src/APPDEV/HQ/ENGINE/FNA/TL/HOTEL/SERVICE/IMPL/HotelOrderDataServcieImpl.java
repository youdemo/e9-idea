...
//                cell.setCellStyle(sheetStyle2);
    ...
    public String sendToSap(Map<String, Object> params) {
        ...
        String sql ="select DEFINETITLECONTENT as ZREINR,max(COSTCENTER) as BUKRS,sum(nvl(amount,0)) as BZ_FEE from CTRIP_SETTLEMENHOTELORDERT a where DEFINETITLECONTENT in("+rqids+") and IsDomestic='true' and paytype='Y' and substr(AccCheckBatchNo,instr(AccCheckBatchNo,'_',-1)+1,6) ='"+lastmonth+"' group by DEFINETITLECONTENT";
        rs.execute(sql);
        while(rs.next()){
        ...
        try {
            ...
            if("Y".equals(STATU)){
                ...
                for(String rqid:rqidarr){
                    sql = "insert into uf_fna_sapfkts(travelno,year,month,feeno,type) values('"+rqid+"','"+year+"','"+month+"','"+DOCNO+"','2')";
                    rs.execute(sql);
                }
