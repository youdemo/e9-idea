...
public class FNA_TL_AP_NodeGetCtripDataAction1111 implements Action{
	
	@Override
	public String execute(RequestInfo info) {
		String workflowID = info.getWorkflowid();// 获取工作流程Workflowid的值
		String requestid = info.getRequestid();
		new BaseBean().writeLog("FNA_TL_AP_NodeGetCtripDataAction requestid:"+requestid);
		...
		if(jdcount==1 || jpcount==1){
			journeyNos = requestid;
			flag = ",";
		}
		if(jpcount>1){
			journeyNos = journeyNos+flag+requestid+"-1";
			flag = ",";
		}
		if(jdcount>1){
			for(int i=1;i<=jdcount;i++){
				if(journeyNos.indexOf(requestid+"-"+i)<0){
					journeyNos = journeyNos+flag+requestid+"-"+i;
					flag = ",";
				}
			}
		}
		new BaseBean().writeLog("FNA_TL_AP_NodeGetCtripDataAction journeyNos:"+journeyNos);

		operGba(requestid,mainid,tableName,journeyNos);
	...
	public boolean operGba(String requestid,String mainid,String tableName,String journeyNos){
        ...
        for(String journeyid:journeyNoArr) {
			...
			new BaseBean().writeLog("FNA_TL_AP_NodeGetCtripDataAction journeyid:"+journeyid);
			String result = ctripUtil.getOrderInfo("", journeyid, EmployeeID, requestid);
							...
							if (!Itinerary.get("FlightOrderInfoList").toString().equals("null")) {
								...
								if (flightOrderInfoList != null && flightOrderInfoList.length() > 0) {
									...
									for (int findex = 0; findex < flightOrderInfoList.length(); findex++) {
										...
										if(!"已成交".equals(OrderStatus)||!"因公".equals(CorpPayType)){
											continue;
										}
						...
						if (Itinerary.length() > 0) {
							...
							if (HotelOrderInfoList != null && HotelOrderInfoList.length() > 0) {
								for (int HotelIndex = 0; HotelIndex < HotelOrderInfoList.length(); HotelIndex++) {
									...
									String OrderDetailStatusName = hotelOrderInfo.getString("OrderDetailStatusName");
									String Corp_payType = hotelOrderInfo.getString("Corp_payType");
									if(!"已成交".equals(OrderDetailStatusName)||!"公费".equals(Corp_payType)){
										continue;
									}
									...
									String ctripENDDA = Util.null2String(hotelOrderInfo.get("EndTime").toString()).substring(0, 10);
									;// 退房日期
