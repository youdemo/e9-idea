...
*/
public class FNA_TL_RE_NodeOperator12Action20200603 implements Action{
	...
	public String execute(RequestInfo info) {
		...
		sql = "select traappli,BtrPER,BTRCon,HoExcStaMark, "
		...
		if(rs.next()) {
			...
			String traappli_ = Util.null2String(rs.getString("traappli"));//关联差旅申请requestid
			...
			if(!"".equals(traappli_)){
				sql = "select BTRBEGDA,BTRENDDA,to_date(BTRENDDA,'yyyy-mm-dd')-to_date(BTRBEGDA,'yyyy-mm-dd')+1 Tripdays,BTRCon,BtrLevel, "
						...
						+ " from  formtable_main_932  where requestid = '" + traappli_ + "'";
				rs1.execute(sql);
				if(rs1.next()) {
					String manager6 = "";
					String FundCenterCODE =  Util.null2String(rs1.getString("FundCenterCODE"));//基金中心key
					String FICTR =  Util.null2String(rs1.getString("FICTR"));//基金中心SAP
					String FundCenterP =  Util.null2String(rs1.getString("FundCenterP"));//基金中心负责人
