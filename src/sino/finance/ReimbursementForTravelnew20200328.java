...
* @version 1.0 差旅费报销单
*/
public class ReimbursementForTravelnew20200328 implements Action{
...
//			zsf = Util.null2String(rs.getString("zsf"));// 付款金额合计 主表----单行文本框
			...
			zhmc = Util.null2String(rs.getString("zhmc")); //帐户名称
			mid = rs.getString("id");
		}
		String zgjdid = Util.null2o(weaver.file.Prop.getPropValue("k3cloudmt", "clbxzgjd"));
		String cwshjdid = Util.null2o(weaver.file.Prop.getPropValue("k3cloudmt", "clbxcwjd"));
		String zgsh = "";//主管审核
		String zgshsj = "";
		String cwsh = "";//财务审核
		String cwshsj = "";
		String zjlsh = "";//总经理审核
		String zjlshsj=nowDate;
		...
		rs.execute(sql);
		if(rs.next()){
			zjlsh = Util.null2String(rs.getString("lastname"));
		}
		String days = "";
		sql = "SELECT datediff( day,'"+ksrq+"','"+jsrq+"' )+1 as days";
		rs.execute(sql);
		if(rs.next()){
            days = Util.null2String(rs.getString("days"));
        }
		if(hl.length()<1) {
			hl = "1";
		}
		...
		sb.append("\"FOUTCONTACTUNITTYPE\": \"\", \"FCONTACTUNITTYPE\": \"BD_Empinfo\", \"FOUTCONTACTUNIT\": { \"FNumber\": \"\" }, \"FCONTACTUNIT\": { \"FNumber\": \""+skrgh+"\" }, \"FPayOrgId\": { \"FNumber\": \"\"  }, \"FPaySettlleTypeID\": {  \"FNUMBER\": \""+gu.getFieldVal("uf_jsfs", "bm", "id", jsfs)+"\"  }, \"FRefundBankAccount\": { \"FNUMBER\": \""+yxzh+"\" }, \"FRefundBankAccountName\":\""+zhmc+"\",\"FRefundBankBranch\":\""+khyx+"\",\"FBankBranchT\": \""+khyx+"\", \"FBankAccountNameT\": \""+zhmc+"\",");
		...
		sb.append("\"FCountry\": \"\",  \"FNProvince\": \"\", \"FNCity\": \"\", \"FNDistrict\": \"\",  \"F_ORA_RSCPROJECT\": { \"FNumber\": \""+gu.getFieldVal("uf_yfxm", "bm", "id", yfxm)+"\" }, \"F_ORA_ENGPROJECT\": { \"FNumber\": \"\" },\"F_ORA_LEADERCHECK\": \""+zgsh+"\", \"F_ORA_GMCHECK\": \""+zjlsh+"\",\"F_ORA_FICHECK\": \""+cwsh+"\",\"F_ORA_LEADERDATE\": \""+zgshsj+"\",\"F_ORA_GMDATE\": \""+zjlshsj+"\", \"F_ORA_FIDATE\": \""+cwshsj+"\", \"FEntity\": [");
